...
def train_one_epoch(
	...
	for images, labels in loader:
		...
		indices = torch.randperm(buffer_vals.size(0))[:64]

		buffer_vals = buffer_vals[indices].to(device)
		buffer_labels = buffer_labels[indices].to(device)

		images = torch.cat([images, buffer_vals], dim=0)
...
def main() -> None:
	...
	buffer_vals, buffer_labels = torch.load("replay_buffer.pt")
	...
	train_loader = split_loaders["split_b"]["train"]
	test_loader = split_loaders["split_b"]["test"]

	split_a_loaders = create_dataloaders_from_saved_splits(
		output_root=args.output_root,
		split_names=["split_a"],
		batch_size=args.batch_size,
		num_workers=args.num_workers,
	...
	test_a_loader = split_a_loaders["split_a"]["test"]
	...
	for epoch in range(1, args.epochs + 1):
		...
		print(
			...
			f"test_loss_on_a={test_loss_a:.4f} test_acc_on_a={test_acc_a:.4f}"
			f"test_loss_on_b={test_loss_b:.4f} test_acc_on_b={test_acc_b:.4f}"
		)

		if test_acc_b > best_acc:
			best_acc = test_acc_b
			torch.save(
				{
					"epoch": epoch,
